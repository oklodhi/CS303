...
template<typename Item_Type>
void Binary_Tree<Item_Type>::isBinarySearch(BTNode<Item_Type>* local_root, bool& result){
	if (result == false) //don't make any recursive calls anymore
		return;
	...
	if (local_root->left && local_root->left != NULL) {
		if (local_root->data < local_root->left->data){
			result = false;
			return;
		}
		isBinarySearch(local_root->left, result);
	}

	if (local_root->right != NULL) {
		if (local_root->data > local_root->right->data) {
			result = false;
			return;
		}
		isBinarySearch(local_root->right, result);
	}
}
